...
{
	double pitchLen = 52.5,pitchWid = 34.0;


    int line_count = 0;
    int max_line_count;
    int min_line_count;
    if ( std::fabs( self().pos().x ) < pitchLen*0.5
         && std::fabs( self().pos().y ) < pitchWid*0.5 )
    {
...
VisualSenderPlayerV1::sendLine( const PObject & line )
{
    ...
    //! left line
    if ( line.pos().x == - pitchLen*0.5 )
    {
        line_normal = M_PI;
        if ( self().pos().x < line.pos().x )
            line_normal = 0.0;
        player_2_line = line.pos().x - self().pos().x;
        line_start = - pitchWid*0.5;
        line_stop = pitchWid*0.5;
        vert = true;
    }
    //! right line
    else if ( line.pos().x == pitchLen*0.5 )
    {
        line_normal = 0.0;
        if( self().pos().x > line.pos().x )
            line_normal = M_PI;
