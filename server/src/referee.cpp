...
{
	//Added by emadolsky
	const int goalLimitTime = 60;

	if (M_stadium.playmode() == PM_PlayOn and M_last_touched)
	{
		if (cyclesFromLastKick > goalLimitTime)
		{
			Side side = M_last_touched->side();
			M_stadium.score( side );
			announceGoal( side == RIGHT? M_stadium.teamRight() : M_stadium.teamLeft() );
			M_after_goal_time = 0;
			M_stadium.placeBall( side == RIGHT ? LEFT : RIGHT, M_stadium.ball().pos() );
			M_stadium.changePlayMode( side == RIGHT ? PM_AfterGoal_Right : PM_AfterGoal_Left );
			cyclesFromLastKick = 0;
		}
		else
		{
			cyclesFromLastKick++;
...
TouchRef::kickTaken( const Player & kicker )
{
	...
	if ( &kicker == M_last_touched )
	{
		Side side = (M_last_touched->side() == LEFT) ? RIGHT : LEFT;
		M_stadium.score( side );
		announceGoal( side == RIGHT? M_stadium.teamRight() : M_stadium.teamLeft() );
		M_after_goal_time = 0;
		M_stadium.placeBall( side == RIGHT ? LEFT : RIGHT, M_stadium.ball().pos() );
