...
void Stadium::reflectBall(bool vertical) {

	double l = ServerParam::PITCH_LENGTH * 0.5;
	double w = ServerParam::PITCH_WIDTH * 0.5;

	PVector vel = M_ball->vel();
	PVector pos = M_ball->pos();


	if (vertical){
		vel.y *= -1;
		pos.y = fabs (pos.y) + (w - fabs(pos.y))*2.0;
		pos.y *=  (M_ball->pos().y > 0 ? 1 : -1);
	}else{
		vel.x *= -1;
		pos.x = fabs (pos.x) +  (l - fabs(pos.x))*2.0;
		pos.x *= (M_ball->pos().x > 0 ? 1 : -1);
	}
	std::cout<< "ballPos: ("<< M_ball->pos().x<<","<< M_ball->pos().y << ")" <<std::endl;
	std::cout<< "ballVel: ("<< M_ball->vel().x<<","<< M_ball->vel().y << ")" <<std::endl;
	M_ball->moveTo(pos, vel, M_ball->accel());
    std::cout<< "After touch ballPos: ("<< M_ball->pos().x<<","<< M_ball->pos().y << ")" <<std::endl;
    std::cout<< "After touch ballVel: ("<< M_ball->vel().x<<","<< M_ball->vel().y << ")" <<std::endl;
    std::cout<<"-------"<<std::endl;
