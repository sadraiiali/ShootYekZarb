                    ...
                    const GameTime & )
{
    ...
    /*
     *  Added by the great Hir ..!
     *  I am the best soccer 2d developer ever!
     *  Emad: nagayeedam!
     */
    ...
    if ( posValid() )
    {
        ...
        if (fabs(M_state.pos_.y) > ServerParam::i().pitchHalfWidth())
		{
        	M_state.pos_.y -= 2 * sign(M_state.pos_.y) * (fabs(M_state.pos_.y) - ServerParam::i().pitchHalfWidth());
        	new_vel.y *= -1;
		}
		if (fabs(M_state.pos_.x) > ServerParam::i().pitchHalfLength())
		{
			M_state.pos_.x -= 2 * sign(M_state.pos_.x) * (fabs(M_state.pos_.x) - ServerParam::i().pitchHalfLength());
			new_vel.x *= -1;
		}
    }
